...
def main():
    ...
    retailerDataFrame: DataFrame = spark.read.option("header", True).option("delimiter", ";")\
    ...
    best10DaysDF: DataFrame = retailerDataFrame.withColumn("TOTAL_SALES", f.col("QUANTITYORDERED") *
                                                           f.col("PRICEEACH")).sort("TOTAL_SALES", ascending=False)\
                                                            .select("ORDERDATE", "TOTAL_SALES")
    retailSample: List[Row] = best10DaysDF.show(10)
    print(retailSample, sep="\n")  # prints each Row to its own line
    ...
    salesDS = spark.createDataFrame(salesList)

    sales2019 = salesDS.where(salesDS.year == 2019).select("euros", "year").first()
    print(f"Sales for 2019: {sales2019.euros}")

    maximumSales = salesDS.sort("euros", ascending=False).select("euros", "year").first()
    ...
    salesDS = spark.createDataFrame(multiSalesList)

    salesDS = salesDS.groupby("year").agg(f.expr("sum(euros)as euros"))
    ...
    retailerStreamingDF: DataFrame = spark.readStream.option("header", True).option("sep", ";")\
                                                                        .csv("streamingData")

    retailerStreamingDF.writeStream.format("csv").option("path", "streamingData") \
        .option("header", True).option("checkpointLocation", "streamingData") \
        .start()

    printTaskLine(6)
    # Task 6: Find the best selling days in the streaming data
    bestDaysDFStreaming = retailerStreamingDF.withColumn("TOTAL_SALES", f.col("QUANTITYORDERED") *
                                                         f.col("PRICEEACH")) \
        .select("ORDERDATE", "TOTAL_SALES").sort("TOTAL_SALES", ascending=False)
