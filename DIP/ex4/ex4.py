...
from pyspark.sql.types import StructType, StructField, DoubleType, Row, FloatType
...
def main():
    ...
    dataDF: DataFrame = dataRDD.toDF(schema)
    ...
    lr: LinearRegression = LinearRegression(featuresCol="features", labelCol="label", predictionCol="prediction",
    ...
    assembledDF: DataFrame = vectorAssembler.transform(dataDF)
    allPredictions: DataFrame = lrModel.transform(assembledDF)
    allPredictions.show()
    ...
    X = [-0.5, 3.14, 7.5]
    X_RDD: RDD[Row] = spark.sparkContext.parallelize(X)
    X_DF: DataFrame = X_RDD.toDF(schema)

    assembledX: DataFrame = vectorAssembler.transform(X_DF)
    predictions: DataFrame = lrModel.transform(assembledX)
    predictions.show()
    ...
    numberPredictionsDF: DataFrame = __unknown__
