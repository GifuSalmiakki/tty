...
def main():
    ...
    weatherDataFrame: DataFrame = spark.read.option("header", True).csv("data/rdu-weather-history.csv")
    ...
    weatherSample: List[Row] = weatherDataFrame.show(3)
    ...
    minTemp: float = weatherDataFrame.select(f.min("temperaturemin")).collect()
    maxTemp: float = weatherDataFrame.select(f.max("temperaturemax")).collect()
    ...
    weatherDataFrameWithYear: DataFrame = weatherDataFrame.withColumn("year", f.year("date"))
    ...
    aggregatedDF: DataFrame = weatherDataFrameWithYear.groupby("year").agg(f.min("temperaturemin"), f.max("temperaturemax"))
    ...
    task6DF: DataFrame = weatherDataFrameWithYear.groupby("year").agg(f.min("temperaturemin"), f.max("temperaturemax"),
                                                                      f.round(f.avg("avgwindspeed"), 2), f.count("date"))
